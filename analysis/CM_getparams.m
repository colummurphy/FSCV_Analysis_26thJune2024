function [parameters,csc_map,event_codes] = ...
    CM_getparams(app, is16ChannelData, subjectid, recordid, ncschannels, varargin)
...
if (is16ChannelData == true)
    anodal_lim = app.getAnodalLimitVal();
    cathodal_lim = app.getCathodalLimitVal();
    vlim = anodal_lim - cathodal_lim;
    sampling_freq = app.getScanFrequency16Ch();
end    
...
Cts_mat_DAPH=Cts_mat(1:2,1:index_ph(end)); 

% new code here
% samplesPerScan = app.getSamplesPerScan16Ch();
% if (is16ChannelData == true & length(Ats_mat) ~= samplesPerScan)

samplesPerScan = app.getSamplesPerScan();
if (length(Ats_mat) ~= samplesPerScan)
    Ats_mat = resampleCV(Ats_mat,length(Ats_mat), samplesPerScan);       %resample templates to match recorded length
end
% end of new code

disp("length of Ats_mat:");
disp(length(Ats_mat));
...
% assignin("base", "params_Vrange", Vrange);
% assignin("base", "params_Vrange_cathodal", Vrange_cathodal);
