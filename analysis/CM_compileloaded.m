...
assignin('base', 'processedIread', processed.Iread);
...
%temp line
%assignin('base','eventArray', processed.Iread(selch(1)).events);
%assignin('base','eventArrayProcessed', plotParam.events);
...
for ii = 1 : length(selch)
    ...
    processed.Iread(chNum).data =... 
            CM_filterDisplay(app, [], [], processed.Iread(chNum).rawdata);
...
%}
  ...
  % get the indices of plots with a channel assigned
  colorPlotIndices = app.getIndicesOfColorPlots(); 

  % clear all color plots
  for i = 1 : 4
    colorPlot = colorPlots(i);
    cla(colorPlot);
  ...
  % for each plot with a channel assigned
  ...
  for ii=1:length(selch)
    ...
    if (app.getIsPcrCalculated() == true)
        ...
        processed.Ipcr{selch(ii)} = ...
         ...
         CM_setguipct(dopaminePlot, processed.Ipcr{selch(ii)},plotParam, parameters,...
                plotParam.colorFSCV(selch(ii),:),'plotnum',ii);
    end
